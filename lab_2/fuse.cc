                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    if(attr->st_size > st.st_size)
      yfs->write(inum, "", st.st_size, attr->st_size-st.st_size);
    else
                 ...
                 struct fuse_file_info *fi)
{
  ...
  yfs_client::status ret;
  yfs_client::inum inum = ino;
  ret = yfs->write(ino, buf, off, size);
...
const static int OBJ_DIRECTORY = 0;
const static int OBJ_FILE = 1;
...
create_inum(int obj)
{
  ...
  if(obj == OBJ_FILE)
    inum = 0x80000000;  // Because of file, not directory
  else if(obj == OBJ_DIRECTORY)
    inum = 0x00000000;
  uint32_t n = mrand48();
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
  ...
  // You fill this in for Lab 2
  yfs_client::inum inum, t_inum;
  if(yfs->isExist(parent, name, inum)){
    return yfs_client::EXIST;
  }else{
    t_inum = create_inum(OBJ_FILE);
    if(yfs->create(t_inum, name) == yfs_client::OK){
      e->ino = t_inum;
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  // You fill this in for Lab 2
  yfs_client::inum inum;
  found = yfs->isExist(parent, name, inum);
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  ...
  yfs_client::status ret = yfs->getAllFile(inum, vec);
  if(ret == yfs_client::OK){
