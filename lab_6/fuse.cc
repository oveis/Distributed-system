                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    if(attr->st_size == 0){
      yfs->write(inum, "", 0, 0);
      st.st_mtime = st.st_ctime = (int)time(NULL);
      st.st_size = 0;
    }else if(attr->st_size > st.st_size){
      yfs->write(inum, '\0', st.st_size, attr->st_size - st.st_size);
      st.st_mtime = st.st_ctime = (int)time(NULL);
...
static const int OBJ_FILE = 1;
static const int OBJ_DIR = 0;
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
  ...
  yfs_client::inum p_eid = parent;
  yfs_client::status st;
  yfs_client::status result; 

  create_inum(OBJ_FILE, inum);
  st = yfs->isExist(parent, str_name, inum);

  if(st == yfs_client::EXIST){
    e->ino = inum;
    result = EEXIST;
  }else if(st == yfs_client::OK){
    yfs->put(parent, inum, str_name);
    getattr(inum, e->attr);
    e->ino = inum;
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  ...
  //test 
  printf("[fuse readdir] vec size:%d\n", vec.size());

  if(ret == yfs_client::OK){
    //test
    printf("[fuse readdir] OK\n");

    for(unsigned int i=0; i<vec.size(); i++){
      //test
      printf("[fuse readdir add] [%s]:%d\n", vec[i].name.c_str(), vec[i].inum);
