...
std::map<unsigned int, unsigned int> reply_window_delete_xid;
                ...
                unsigned int xid_rep, char **b, int *sz)
{
        ...
        std::list<reply_t> list_rt = rpcs::reply_window_[clt_nonce];
        std::list<reply_t>::iterator it;
        rpcs::rpcstate_t ret;

        if(list_rt.empty()){
                ret = NEW;
                reply_window_delete_xid[clt_nonce] = 0;
        }else{
                bool exist_f = false;
                for(it=list_rt.begin(); it!=list_rt.end(); it++){
                        reply_t rt = *it;

                        if(rt.xid == xid){
                                if(rt.cb_present == true){
                                        *b = rt.buf;
                                        *sz = rt.sz;
                                        ret = DONE;
                                }else if(rt.cb_present == false){
                                        ret = INPROGRESS;
                                }
                                exist_f = true;
                                break;
                ...
                if(!exist_f && reply_window_delete_xid[clt_nonce] >= xid){
                        ret = FORGOTTEN;
                }else if(!exist_f){
                        ret = NEW;
                }
        ...
        if(ret == NEW){
                reply_t new_rt(xid);
                list_rt.push_back(new_rt);
        }

        // 3) trim the window of remembered requests and reply values
        for(it = list_rt.begin(); it != list_rt.end(); it++){
                reply_t rt = *it;
                if(rt.xid <= xid_rep){
                        ...
                        if(reply_window_delete_xid[clt_nonce] < xid_rep){
                                reply_window_delete_xid[clt_nonce] = xid_rep;
                        }
                }
        }

        rpcs::reply_window_[clt_nonce] = list_rt;
        return ret;
}
