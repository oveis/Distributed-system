                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    if(st.st_size == 0){
      ...
      yfs->write(inum, "", st.st_size, st.st_size);
    yfs->setattr(inum, st);
    getattr(inum, st);
                 ...
                 struct fuse_file_info *fi)
{
  ...
  string st(buf, size);
  yfs_client::status ret;
  ret = yfs->write(inum, st, off, size);
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
  ...
  st = yfs->put(parent, inum, str_name);
  ...
  /*
  yfs_client::status st = yfs->isExist(parent, str_name, inum);
  int result;
  if(st == yfs_client::EXIST){
    result = EEXIST;
  }else {
    create_inum(OBJ_FILE, inum);
    if(yfs->put(parent, inum, str_name) != yfs_client::IOERR){
      getattr(inum, e->attr);
      result = yfs_client::OK;
    }
  }
  e->ino = inum;
  return result;
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  yfs_client::inum inum;
  std::string str_name(name);
  if (yfs->isExist(parent, str_name, inum) == yfs_client::EXIST){
    getattr(inum, e.attr);
    e.ino = inum;
    fuse_reply_entry(req, &e);
  }else{
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  ...
  yfs_client::status ret = yfs->getDirEnts(inum, vec);
  if(ret == yfs_client::OK){
     ...
     mode_t mode)
{
  ...
  yfs_client::inum temp_inum;

  // Check this file name is existed already in the parent's directory   
  if(yfs->isExist(parent, name_str, temp_inum) == yfs_client::EXIST){
    fuse_reply_err(req, EEXIST);
  }else{
    // Create new directory's inum
    yfs_client::inum new_inum;
    create_inum(OBJ_DIRECTORY, new_inum);
    e.ino = new_inum;

    // create a new directory with name @name in @parent
    if(yfs->put(parent, e.ino, name_str) != yfs_client::IOERR){
      
      // Set new directory's attributes
...
fuseserver_unlink(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  std::string str_name(name);
  int ret = yfs->unlink(parent, str_name);
  /*
  switch(ret){
  case yfs_client::OK:
