...
int extent_server::write(extent_protocol::extentid_t id, std::string buff, long long unsigned int  off, size_t size, int &)
{
  ...
  if(it != inods_map.end()){
	...
	//temp = content.replace(0, size, buff.substr(0, size));
      }else if(off + size > content.length()){
	printf("case 2\n");
	content.append(off+size-content.length(), '\0');
	temp = content.replace(off, size, buff.substr(0, size));
      }else if(off >= content.length()){
	printf("case 3\n");
	temp = content.append(off - content.length(), '\0').append(buff.c_str(), size);
      }else{
	temp = content.replace(off, size, buff.substr(0, size));
      }
      files_map[id] = temp;
      ...
      cout << "i_node input size : " << attr.size << endl;
      cout << "temp    :[" << temp << "]"<< endl;
      cout << "files_map : [" << files_map[id] << "]" << endl;
...
int extent_server::read(extent_protocol::extentid_t id, size_t size, long long unsigned int off, std::string& buf)
{
  ...
  if(it != inods_map.end()){
    ...
    if(f_it != files_map.end()){
      //string content = f_it->second;
      int file_size = inods_map[id].size;
      string content = f_it->second;

      cout << "file size from inod : " << file_size << endl;
      cout << "read content : [" << f_it->second.substr(0, file_size) << "]" << endl;
      cout << "read content size : " << file_size << endl;
      printf("read off : %d, content_leng : %d\n", off, sizeof(content));
	...
	printf("read case 3\n");
      ...
      cout << "[" << buf << "]" << endl;
      printf("bus size : %d\n", buf.size());
      cout << "file length : " << inods_map[id].size << endl;
